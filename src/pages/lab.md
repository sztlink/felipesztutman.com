...
## Study 7. Twelve frames per second of unusable video

The four-bit cache left the card with room, so the next question was speed. Four levers remained and three of them paid. The attention kernel the release ships carries no sm89 kernels at all, so every number in this practice until now was measured with fast attention switched off, and rebuilding it from source for the right architecture returns 1.11 times on the denoise pass. Compiling the VAE decoder returns 1.12 times for 4.4 GB of extra peak, which is a bad trade when that memory buys context instead. And the decoder itself turned out to hide the real lever. The server hardcodes the heavy VAE and never reads the `use_taehv` flag that sits in both of its configs, the fourth dead flag I have found in this codebase, while the tiny distilled decoder shipped alongside it decodes the same three latent frames **25.6 times faster** at 11.3 million parameters.

It does not drop in. The tiny decoder allocates its temporal memory fresh on every call, and a server that decodes one block at a time would begin each block with no memory of the last, leaving a seam every twelve frames. Measured against the heavy decoder as a control, frame to frame difference at block boundaries divided by the same difference inside blocks reads 1.023 for the heavy decoder and 3.008 for the tiny one called per block. Hoisting that memory out of the graph traversal and threading it through the server's own cache contract brings it to 1.208.

Sweeping the denoising steps then gives a clean linear model of the frame. Block time is the step count times 0.717 seconds plus a fixed cost, and that fixed cost falls from 0.841 seconds to 0.203 when the tiny decoder replaces the heavy one. The fit predicted 0.920 seconds for a one-step run and the measurement came back 0.929. Stacking one step, fast attention and the tiny decoder produced **12.73 fps at 832x480, peaking at 14.06 GB, with 0.93 seconds of latency per block**, on a card that cannot load the full precision model at all.

Felipe looked at it and said it was unusable. The skater had fallen apart.

The cause took one comparison that had been sitting on my disk for hours without anyone being asked to look at it. Across four step counts at full decoder quality the verdict was four steps good, three acceptable, and anything below that unusable. So the step count is the cause, the decoder is exonerated, and the whole table reprices. At three steps the block takes 2.35 seconds, which is **5.0 fps**, and that is the real ceiling of this model on this card at quality a person will accept. The 12.73 was real and it was frames per second of video nobody can use.

I had optimised three things in sequence, measuring speed at every step and quality at none.

![Solar sphere, first act](/img/lab/study7-solar-sphere.jpg)

*Krea Realtime 14B on one RTX 4090, four bit weights, four bit KV cache, twelve frames of context, four denoising steps. The prompt asks for a vast sphere of solar plasma with filaments arcing from the limb, in an extreme wide shot with a slow push in.*

## Study 8. The model ships its own prompt specification

Two afternoons went into a failure that had nothing to do with quantisation, and both halves of it are worth writing down.

The first half was mine. Prompt embeddings on this stack are built on the CPU, because the text encoder constructs itself in full precision at 22.7 GB and will not sit beside the video model. Three to five minutes per prompt is a real tax on the one thing you most want to iterate, so I rewrote the encoder to build in half precision and run on the card, which took it to a quarter of a second per prompt. The tokeniser pads every prompt to 512 positions, the encoder produces values at the padded ones, and the original script zeroes everything past the real tokens before saving. My rewrite skipped that line. The model then attends across 512 positions of which most carry padding noise, conditioning collapses, and the output becomes flat brown mush with a lattice through it. Latents stay finite. Throughput is normal. No error is raised anywhere. Every instrument I had said the run was healthy.

I found it because Felipe told me to go and look at what I was generating. I had reported an entire sequence of results without once opening a frame.

The second half was the prompts themselves. Asked for the seven abstract acts of a fire installation, the model returned moving texture rather than composed image, and I diagnosed that as too little form and rewrote them with more. The actual answer was in the repository the whole time. `wan/utils/prompt_extend.py` carries the system prompt this model family uses to rewrite short inputs into good ones, which makes it the documented specification. Declare a style first. Describe the subject with attributes, then the background separately. Emphasise motion and camera movement with direct verbs. End with the shot scale. The four worked examples run four to six hundred characters.

Mine were a hundred and twenty, with no style and no shot scale. Same model, same seed, same quantised stack, same seven acts, rewritten to the documented format, and the first act becomes a sphere of solar plasma against a starfield with light arcing from its limb.

There is a second finding underneath, and it belongs to Felipe rather than to me. Watching the first realistic tests he said the results were good but too realistic, and that the largest gain available was probably in the prompt, because something more abstract might let us loosen the brakes. Abstraction forgives artefacts. A photoreal body needs four denoising steps because every defect reads as broken anatomy, while an abstract form has no referent for the eye to convict. Five aesthetic directions were rendered at three step counts each, and three were approved. The two that failed were the two that ask for **matter with weight**, embers with mass and molten rock. The three that hold are surface, light and fluid. What abstraction buys here is release from having to get physics right, and that release doubles as a performance lever, the only one on the list that improves the work rather than degrading it.

## Where this goes

The image stack generates a 1024px image in 5.5 seconds on the 4090, measured warm, 8 steps, fp8. The video stack runs the 14B at 5.0 fps on this card at quality that survives being looked at, and at 12.73 fps if you are willing to ship video nobody can use. That gap is the honest state of the work. Every road I named a week ago has now been walked, the fused kernels, the quantized cache and the attention kernel, and none of them moves the line, because the line is set by how many denoising steps the content needs. What moves it is the content itself, which is why the most useful measurement of the past week came from an aesthetic decision rather than a technical one. The harnesses that produced the tables above are the gates. They are open source, and the numbers they produce are the kind I would want to read from anyone else.
