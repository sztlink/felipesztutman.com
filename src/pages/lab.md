...
The image stack generates a 1024px image in 5.5 seconds on the 4090, measured warm, 8 steps, fp8. The video stack now runs the 14B at 2.81 fps and its 1.3B sibling at 8.8 fps, both at 832x480 in four bits on the same card. Real time for an installation means closing the distance between those numbers and the speed of presence, and the road runs through kernel fusion, a 4-bit KV cache, and honest fidelity gates at every stage. The harnesses that produced the tables above are the gates. They are open source, and the numbers they produce are the kind I would want to read from anyone else.
